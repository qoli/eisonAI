...
| 技能领域 | 具体要求 | 相关文件 |
|---------|---------|---------|
| **Swift/SwiftUI** | `async/await`, `@MainActor`, `@StateObject`, Combine | `iOS (App)/Features/*` |
| **Safari Extension** | manifest.json, CSP, Native Messaging, Content Scripts | `Shared (Extension)/Resources/*` |
| **JavaScript (ES6+)** | Promise/async, Web Worker, WebGPU API | `popup.js`, `worker.js` |
| **WebLLM/MLC** | 模型配置, WASM 集成, conv_template | `webllm.js`, `mlc-chat-config.json` |
| **CloudKit** | CKContainer, CKRecord, 私有数据库操作 | `RawLibraryCloudDatabase.swift` |
| **iOS 开发** | App Groups, Entitlements, Share Extension | `shareExtension/*` |

### 6.2 建议掌握

- **Tokenization (BPE)** - 理解 SwiftikToken 实现
- **MVVM 架构** - 维护现有架构并重构
